...
final class AppState: ObservableObject {
    ...
    func exportDiagnosticBundle(to url: URL) throws {
        let configuration = currentConfigurationSnapshot()
        let configurationData = try encodedConfigurationData(configuration)
        let auditSummary = auditSummary(for: configuration)
        let healthReport = configurationHealthReport
        let networkInterfaces = NetworkInventoryService.currentIPv4Interfaces()

        var files: [DiagnosticBundleFile] = [
            DiagnosticBundleFile(
                path: "README.txt",
                text: diagnosticReadMeText()
            ),
            DiagnosticBundleFile(
                path: "App Info.txt",
                text: diagnosticAppInfoText()
            ),
            DiagnosticBundleFile(
                path: "Configuration/Current Configuration.launchcontrol",
                data: configurationData
            ),
            DiagnosticBundleFile(
                path: "Configuration/Schedule Check.txt",
                text: diagnosticAuditText(auditSummary)
            ),
            DiagnosticBundleFile(
                path: "Configuration/Configuration Health.txt",
                text: diagnosticHealthText(healthReport)
            ),
            DiagnosticBundleFile(
                path: "Network/Network Interfaces.txt",
                text: diagnosticNetworkText(networkInterfaces)
            )
        ]

        let logFiles = DiagnosticBundleService.recentLogFiles(maxCount: 5)

        if logFiles.isEmpty {
            files.append(
                DiagnosticBundleFile(
                    path: "Logs/No Recent Logs.txt",
                    text: "No readable operational log files were available when this diagnostic bundle was created.\n"
                )
            )
        } else {
        ...
        try DiagnosticBundleService.writeBundle(
            files: files,
            to: url
        )

        lastMessage = "Exported diagnostic bundle: \(url.lastPathComponent)"
        logger.info(lastMessage)
    }
    ...
    private func encodedConfigurationData(_ configuration: LaunchControlConfiguration) throws -> Data {
        let encoder = JSONEncoder()
        encoder.outputFormatting = [
            .prettyPrinted,
            .sortedKeys,
    ...
    private func diagnosticReadMeText() -> String {
        """
        Launch Control Center Diagnostic Bundle

        This bundle was generated by Launch Control Center for troubleshooting and support.
        It includes a configuration export, schedule check report, configuration health report,
        network interface snapshot, app/system information, and recent operational logs.

        This bundle may include project names, Action names, Event timing, UDP destinations,
        Syslog messages, local network interface addresses, and operational log history.

        Created: \(Self.diagnosticDateFormatter.string(from: Date()))
        
        """
    }

    private func diagnosticAppInfoText() -> String {
    ...
    private func diagnosticAuditText(_ summary: ConfigurationAuditSummary) -> String {
        var lines: [String] = [
            "Schedule Check: \(summary.statusText)",
            "Project: \(summary.projectName)",
            "Actions: \(summary.actionCount)",
            "Events: \(summary.eventCount)",
            "Recurring Series: \(summary.recurringSeriesCount)",
            ""
        ]

        if summary.issues.isEmpty {
            lines.append("No schedule check issues were reported.")
        } else {
            lines.append("Issues:")

            for issue in summary.issues {
                lines.append("- [\(issue.severity.displayName.uppercased())] \(issue.message)")
            }
        }
    ...
    private func diagnosticHealthText(_ report: ConfigurationHealthReport) -> String {
        var lines: [String] = [
            "Configuration Health: \(report.title)",
            report.summary,
            ""
        ]

        if report.issues.isEmpty {
            lines.append("No configuration health issues were reported.")
        } else {
            lines.append("Issues:")

            for issue in report.issues {
                lines.append("- [\(issue.level.displayName)] \(issue.title): \(issue.detail)")
            }
        }
    ...
    private func diagnosticNetworkText(_ interfaces: [NetworkInterfaceSnapshot]) -> String {
        var lines: [String] = [
            "Network Interfaces",
            "Created: \(Self.diagnosticDateFormatter.string(from: Date()))",
            ""
        ...
        if interfaces.isEmpty {
            lines.append("No local IPv4 interfaces were detected.")
        } else {
            for interface in interfaces {
                lines.append("Interface: \(interface.name)")
                lines.append("  IPv4 Address: \(interface.ipv4Address)")
                lines.append("  Availability: \(interface.availabilityText)")
                lines.append("  Netmask: \(interface.netmask ?? "None")")
                lines.append("  Broadcast: \(interface.broadcastAddress ?? "None")")
                lines.append("  Broadcast Capable: \(interface.supportsBroadcastText)")
                lines.append("")
            }
        }
    ...
    private func diagnosticScheduleStatusText() -> String {
        ...
        case (false, false):
            return "Show and Utility Events disabled"
        }
    }
